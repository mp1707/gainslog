...
import { createStyles } from "./FloatingSegmentedControl.styles";

interface FloatingSegmentedControlProps {
  selectedIndex: number;
  onSegmentChange: (index: number) => void;
  segments: string[];
}

export const FloatingSegmentedControl: React.FC<
  FloatingSegmentedControlProps
> = ({ selectedIndex, onSegmentChange, segments }) => {
  ...
  useEffect(() => {
    if (containerWidth.value > 0 && segmentWidths.current.length > 0) {
      ...
      const position = segmentWidths.current
        .slice(0, selectedIndex)
        .reduce((acc, width) => acc + width, 0) + (theme.spacing.xs / 2); // Add container padding offset
